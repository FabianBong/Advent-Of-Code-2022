class NonBinTree:
    ...
    @staticmethod
    def part_1_answer(start_node):
        score = 0
        if start_node.get_value().get_type()==1:
                if start_node.get_value().get_size()<=100000:
                    #print(start_node.get_value().get_size())
                    score += start_node.get_value().get_size()
        for node in start_node.nodes:
            answ = NonBinTree.part_1_answer(node)
            if answ != None:
                score += answ
        return score
...
print(NonBinTree.part_1_answer(cur))
#a = NonBinTree(0)
#a.add_node(1)
#a.add_node(3)
#a.add_node(4)
